...
class Edit extends React.Component {
        ...
        var prep_by=localStorage.getItem('userData') ? JSON.parse(localStorage.getItem('userData')).username : '';
        this.state = {
            loading: false,
            invoice_no: '',
...
handleChange = (index, evnt) => {
   ...
   submitButton = async (e) => {
    ...
    const file_id = arrayPath[2];
        ...
        Api.postData(`/distributor-dispatch/edit/${file_id}`, payload).then(res => {
    ...
    async componentDidMount() {
        ...
        Api.getData(`/distributor-dispatch/edit/${file_id}`, '').then((res) => {
          
            var distributorcode= res.data.result.distributorcode;
            Api.getData(`/distributor-dispatch/getDealers/${distributorcode}`, '').then((res) => {
                let dealers = res.data.result;
                this.setState({ dealers: dealers });
             })
             Api.getData(`/distributor-dispatch/getDlrOrdrs/${res.data.result.dealercode}`, '').then((res) => {
                let orders = res.data.result;
               this.setState({ orders: orders });
             })
             var line = res.data.result.desp_details; var n = line.length; var itm_detail = new Array();
            for (let i = 0; i < n; i++) {
                const new_row = { product: line[i].productcode,product_name: line[i].get_product_name.productname,
            ...
            this.setState({
                invoice_no: res.data.result.despinvoiceno,
                invoice_date: res.data.result.invoicedate,
                distributor: res.data.result.distributorcode,
                dealer: res.data.result.dealercode,
                order_no:res.data.result.orderno,
                status: res.data.result.dispatchstatus,
                                    ...
                                    <div className="page-title-box d-flex align-items-center justify-content-between">
                                        ...
                                        <div className="page-title-right">
                                            <ol className="breadcrumb m-0">
                                                ...
                                                <li className="breadcrumb-item active">Edit Distributor Dispatch</li>
                                            </ol>
                                        </div>
                                    </div>
                                </div>
