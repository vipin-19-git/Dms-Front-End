 ...
 async componentDidMount()
  {
   ...
   Api.getData(`/model/edit/${file_id}`,'').then((res)=>{
   
    console.log(res);
    this.setState({product:res.data.result.productcode})
    this.setState({model_code:res.data.result.modelcode})
    this.setState({model_name:res.data.result.modelname})
    this.setState({cylinder:res.data.result.cylinder})
  ...
  submitButton = async (e) => {
    ...
    let payload = {product: this.state.product,model_code:this.state.model_code, model_name: this.state.model_name,cylinder: this.state.cylinder};
     Api.postData('/model/update', payload).then(res =>{
                ...
                <div className="page-title-box d-flex align-items-center justify-content-between">
                 <h4 className="mb-0"></h4>
                  <div className="page-title-right">
                   ...
                   <li className="breadcrumb-item active">Edit Model</li>
                   </ol>
                   </div>
                   </div>
                   </div>
