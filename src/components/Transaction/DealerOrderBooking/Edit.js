...
class Edit extends React.Component {
    ...
    async componentDidMount() {
        ...
        Api.getData(`/dlrbooking/edit/${id}`, '').then((res) => {
            var line = res.data.result.order_details; var n = line.length; var itm_detail = new Array();
            Api.getData(`/dlrbooking/getDlrDist/${res.data.result.statecode}`, '').then((res) => {
               this.setState({ dealers: res.data.result.dealers });
               this.setState({ distributors: res.data.result.distributors });
            })
            const state_models = this.state.models;
            for (let i = 0; i < n; i++) {
               const new_row = { product: line[i].productcode, model: line[i].modelcode, qty: line[i].quantity,amt:line[i].price };
            ...
            this.setState({
               ...
               order_date: res.data.result.bookingdate,
               state_nm: res.data.result.statecode,
               dealer: res.data.result.dealercode,
               distributor: res.data.result.distributorcode,
               status: res.data.result.status,
               prep_by: res.data.result.preparedby,
    ...
    submitButton = async (e) => {
        ...
        Api.postData(`/dlrbooking/edit/${id}`, payload).then(res => {
                                    ...
                                    <div className="page-title-box d-flex align-items-center justify-content-between">
                                        ...
                                        <div className="page-title-right">
                                            <ol className="breadcrumb m-0">
                                                ...
                                                <li className="breadcrumb-item active">Edit Dealer Order Booking</li>
                                            </ol>
                                        </div>
                                    </div>
                                </div>
