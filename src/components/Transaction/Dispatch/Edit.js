...
handleChange = (index, evnt) => {
   ...
   submitButton = async (e) => {
        ...
        Api.postData(`/dispatch/edit/${file_id}`, payload).then(res => {
    ...
    async componentDidMount() {
        ...
        Api.getData(`/dispatch/edit/${file_id}`, '').then((res) => {
            ...
            var tarnsportercode= res.data.result.transportcode;
            Api.getData(`dispatch/getDistOrder/${distributorcode}`, '').then((res) => {
                let orders = res.data.result.orders;
                let transporters = res.data.result.transporters;
                this.setState({ orders: orders });
                this.setState({ transporters: transporters });
             })
             Api.getData(`dispatch/geVehicle/${tarnsportercode}`, '').then((res) => {
                let vehicles = res.data.result;
               this.setState({ vehicles: vehicles });
             })
            for (let i = 0; i < n; i++) {
                                    ...
                                    <div className="page-title-box d-flex align-items-center justify-content-between">
                                        ...
                                        <div className="page-title-right">
                                            <ol className="breadcrumb m-0">
                                                ...
                                                <li className="breadcrumb-item active">Edit Dispatch</li>
                                            </ol>
                                        </div>
                                    </div>
                                </div>
