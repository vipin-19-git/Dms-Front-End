...
class Edit extends React.Component {
   ...
   async componentDidMount() {
      ...
      Api.getData(`/dobooking/edit/${file_id}`, '').then((res) => {
         var line = res.data.order_details; var n = line.length; var itm_detail = new Array();
         Api.getData(`/dobooking/getdistributor/${res.data.statecode}`, '').then((res) => {
            this.setState({ distributors: res.data });
         })
         const state_models = this.state.models;
         for (let i = 0; i < n; i++) {
            const new_row = { product: line[i].productcode, model: line[i].modelcode, qty: line[i].quantity };
         ...
         this.setState({
            ...
            order_date: res.data.bookingdate,
            state_nm: res.data.statecode,
            distributor: res.data.distributorcode,
            bstatus: res.data.status,
            prep_by: res.data.preparedby,
            remarks: res.data.remarks,
            line_item: itm_detail
         })
      })
   ...
   submitButton = async (e) => {
      ...
      let payload = {
         ...
         , distributor: this.state.distributor, status: this.state.bstatus, prep_by: this.state.prep_by, remarks: this.state.remarks,
         line_item: this.state.line_item
      };
      Api.postData('/dobooking/update', payload).then(res => {
                           ...
                           <div className="card">
                              <h5 className="card-header" style={{ backgroundColor: 'rgb(163 168 178 / 47%)' }}>
                                 ...
                                 <Button variant="success" disabled={this.state.loading} onClick={!this.state.loading ? this.submitButton : null}
                                    style={{ float: 'right' }} > <FaSave /> {this.state.loading ? 'Updating…' : 'Update'}</Button>
                                    ...
                                    <div className="row" style={{marginTop:'-20px'}}>
                                       ...
                                       <div className="col-md-3">
                                          <div className="mb-3">
                                             ...
                                             <select ref="parentOption" className="form-control" id="distributor" value={this.state.bstatus}
                                                name="bstatus" onChange={this.handleInput}>
                                       ...
                                       <div className="col-md-3">
                                          <div className="mb-3">
                                             ...
                                             <input type="text" className="form-control" id="prep_by" name="prep_by" value={this.state.prep_by} onChange={this.handleInput} />
                                          </div>
                                       </div>
                                                ...
                                                {this.state.line_item.map((data, index) => {
                                                   ...
                                                   return (
                                                      <tr key={index}>
                                                         ...
                                                         <td><input type="text" value={qty} onChange={(evnt) => (this.handleChange(index, evnt))} name="qty" className="form-control" /> </td>
